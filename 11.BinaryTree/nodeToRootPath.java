...
public class nodeToRootPath {
  ...
  public static boolean find(Node node, int data){
    ...
    boolean leftFind = find(node.left , data);
    if(leftFind == true) return true;

    boolean rightFind = find(node.right , data);
  ...
  public static ArrayList<Integer> nodeToRoot(Node node, int data){
      ...
      ArrayList<Integer> leftAns = nodeToRoot(node.left , data);
      if(leftAns.size() > 0){
        leftAns.add(node.data);
        return leftAns;
      }

      ArrayList<Integer> rightAns = nodeToRoot(node.right , data);
      if(rightAns.size() > 0){
